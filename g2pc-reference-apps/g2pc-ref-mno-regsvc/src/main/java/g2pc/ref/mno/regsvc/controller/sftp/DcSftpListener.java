package g2pc.ref.mno.regsvc.controller.sftp;
...
public class DcSftpListener {
    ...
    public void handleMessageInbound(Message<File> message) {
        try {
            ...
            log.info("Received Message from inbound directory of dp-2: {}", file.getName());
            if (ObjectUtils.isNotEmpty(file) && file.getName().contains(".json")) {
                ...
                try {
                    ...
                    requestHandlerService.buildCacheRequest(
                            objectMapper.writeValueAsString(requestDTO), cacheKey, CoreConstants.SEND_PROTOCOL_SFTP, sunbirdEnabled);
